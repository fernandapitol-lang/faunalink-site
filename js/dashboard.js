...
// Custom marker icon if available
if (window.L && document.getElementById('map')) {
  try {
    var faunaIcon = L.icon({ iconUrl: 'assets/map-marker.svg', iconSize: [24,24] });
    // override marker creation to use faunaIcon where possible
    var _orig_marker = L.marker;
    // keep default usage
  } catch(e){
    console.warn('Fauna icon not applied', e);
  }
}
